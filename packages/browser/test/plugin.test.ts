...
import plugin from "@opencode-ai/browser"
...
const fixture = Effect.gen(function* () {
  ...
  const opencode = yield* OpenCode.create({
    ...
    config: {
      directory: config,
      project: false,
      content: JSON.stringify({ permissions: [{ action: "browser", resource: "*", effect: "allow" }] }),
    },
    models: { fetch: false },
...
test("attachment ownership, cancellation, and plugin unload release pending browser work", () =>
  Effect.gen(function* () {
    ...
    yield* host.opencode.plugin({ id: plugin.id, effect: () => Effect.void })
...
test("commands use published state and permissions, and RPC results render text and screenshot bytes", () =>
  Effect.gen(function* () {
    const host = yield* fixture
    const options = { location: host.location }
    ...
    yield* host.rpc.result(
      { ...attached.input, requestID: open.requestID, outcome: { type: "success", result: { type: "state", state } } },
      options,
    )
